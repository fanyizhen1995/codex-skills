...
description: 当工程类 Codex 任务出现长上下文、测试日志/构建日志/远程日志过大、exec_command/write_stdin 输出反复进入上下文、频繁 compaction、需要证明 token 节省或上下文压缩效果时使用。
---
...
- 用户要求节省 token、分析上下文膨胀、证明 skill 的正确性或效果。
