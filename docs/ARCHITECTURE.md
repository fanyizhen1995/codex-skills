...
- 运行时 SQLite 连接必须在打开数据库前持有共享 maintenance lock；`rebuild-db` 持有同一把锁的独占模式完成 quiescence 检查、构建、交换和回滚，Supervisor heartbeat 持久化也使用共享模式。现有 live DB 的 rebuild 仅支持 Linux/POSIX SQLite Unix VFS 与 `fcntl` byte locks；SQLite checkpoint 是权威安全门，checkpoint 后生成的独立 DELETE-mode DB 才能用于一次原子替换回滚，原始 DB/WAL/SHM triad 只保留作 forensic evidence。
