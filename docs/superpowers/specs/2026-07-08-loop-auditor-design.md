...
因此需要引入独立第三方 `Auditor`，作为 loop 状态机的一等阶段，对连续多轮执行效果进行周期性审计，并在必要时硬阻塞后续普通开发或资料拓展。

## 用户已确认决策

- Auditor 必须是独立第三方审计 agent，不由 Planner、Generator 或 Evaluator 兼任。
- Auditor 拥有硬阻塞权：如果输出 `must_fix`，loop 必须先整改，不能继续普通开发或资料拓展。
- 首期采用“状态机一等阶段 Auditor”，而不是只写报告，也不是外部 daemon。
...
1. 在 loop 中加入 Auditor 阶段，形成 `Planner -> Generator -> Evaluator -> Auditor -> Planner` 的可审计循环。
...
Auditor 是独立第三方 agent，但它不直接写 git 工作树，也不直接修改 run state。执行方式是：
...
在现有 loop phase 基础上新增 Auditor 相关 phase：
...
Parent Planner
  ...
  -> Audit trigger check -> Auditor when due
  -> Parent Planner
  -> ...
  -> Auditor before human merge
...
- `demand-development`：每完成 3 个 child 后触发一次；进入 human merge 前必须触发一次。
- `autonomous-knowledge`：每完成 1 个 autonomous round 后触发一次；轻量入库任务可按每 3 个入库 commit 触发一次。
- `meta-loop`：阶段切换前必须触发一次，例如从需求开发切换到资料拓展。
...
Auditor 维护 loop 相关 skill inventory，避免流程沉淀无限膨胀。
...
- 如果某一类 loop 相关 skill 超过 7 个，Auditor 至少输出 `should_fix`。
...
2. Audit trigger policy
   - demand loop 每 3 个 passed child 触发。
   - autonomous loop 每 1 个 round 触发。
...
4. Tunnel vision detection
   - 同一文件/测试连续反复修改超过阈值时生成 finding。
   - 对低价值问题消耗过多轮次时生成 `refocus` 或 `stop_early`。
   - 纯格式偏好不能升级为 `must_fix`。

5. Skill inventory governance
   - 超过阈值或存在重复 skill 时生成 `should_fix`。
...
- 准备 parent run，3 个 child passed。
- 运行 auditor。
...
- 构造 8 个 loop 相关 skill，其中 3 个职责重叠。
...
### Phase 1：Auditor contract 和只读报告

- 新增 audit report schema。
- 新增 `run-audit` CLI。
- 读取 run artifacts，输出报告。
- Dashboard 展示最近审计。
- 不改变现有 loop 调度。
