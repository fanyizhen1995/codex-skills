...
### 前端方案 / Mock 验收规则
凡是涉及前端页面、看板、交互流程或可视化展示的方案，都必须同步提供并维护 mock 页面设计：

1. 设计 spec 必须引用对应 mock 文件路径，mock 不能只是示意图，必须作为可实现的展示契约。
2. spec 变更导致前端展示字段、布局、状态或交互变化时，必须同步更新 mock；不能出现 spec 与 mock 不一致。
3. mock 中出现的每个状态、指标、列表、按钮、标签和文案，都必须在 spec 中有数据来源、生产者、API/文件契约和测试方案；未实现或无数据时，前端必须显示 `暂无数据`、`未启用` 或 `不可用`，不能显示假成功。
4. 交付验收必须包含“实现是否覆盖 mock 页面”的检查项。涉及前端的 evaluator 需要模拟用户查看页面，验证实现与 mock 的关键结构、内容状态和交互一致。
5. 如果开发中发现某个 mock 展示项无法按契约实现，必须先修改 spec 和 mock，说明该项被移除、降级或改为不可用状态，再继续实现。
