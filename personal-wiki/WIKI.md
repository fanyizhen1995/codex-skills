...
| RSS article ingest | Treat RSS/Atom as a discovery index; fetch entry links over HTTP first, use Playwright only as fallback, keep feed and article metadata in raw, then curate minimal wiki updates. | `使用 personal-wiki-manager，目标 domain: <domain>，入库 RSS <feed-url>：解析 entry，HTTP 抓正文，必要时 Playwright fallback，raw 保 feed/article 元数据，最后 index+validate。` |
| RSS crawl smoke | Verify a feed and one article URL before enabling scheduled ingestion; compare HTTP and Playwright body extraction and report artifacts. | `使用 personal-wiki-manager，验证 RSS <feed-url> 正文抓取：选 1 条 entry，分别跑 HTTP 和 Playwright，报告正文长度、命中关键词和产物路径。` |
...
| RSS 正文入库 | RSS/Atom 只作为发现索引；先解析 entry，再用 HTTP 抓正文，必要时用 Playwright fallback；raw 保留 feed 和 article 元数据。 | `使用 personal-wiki-manager，目标 domain: <domain>，入库 RSS <feed-url>：解析 entry，HTTP 抓正文，必要时 Playwright fallback，raw 保 feed/article 元数据，最后 index+validate。` |
| RSS 抓取验证 | 启用定时入库前先验证 feed 和单篇正文抓取；对比 HTTP 与 Playwright 结果。 | `使用 personal-wiki-manager，验证 RSS <feed-url> 正文抓取：选 1 条 entry，分别跑 HTTP 和 Playwright，报告正文长度、命中关键词和产物路径。` |
