...
captured_at: '2026-07-07T23:35:30.902366+00:00'
content_hash: b4db35f5e2c08127029554f20541bca8156e8f93b72e41903f3e5697231d5d89
---
...
Two FP8 tests I originally included were **verified failing** on the AMD lane
and removed (kept CUDA-only) rather than register-and-disable:
- `moe/test_triton_moe_channel_fp8_kernel.py` — per-channel FP8 fused_moe, 7/8 subtests pass; one config exceeds tolerance on ROCm.
- `quant/test_fp8_kernel.py` — FP8 quant kernel numerics diverge on ROCm.

(These are documented as AMD-excluded on the ROCm dashboard in ROCm/sglang-ci#311.)

## Test plan

Both registered files confirmed **PASS** on the AMD MI325 lane
(`stage-b-test-1-gpu-small-amd`), per-file result lines pulled from the
partition logs of https://github.com/sgl-project/sglang/actions/runs/28817203900 :
- `test_customized_info_streaming.py` ✅ (54s)
- `test_moe_lora_info.py` ✅ (7s)

The suite shows a run-level failure only from a pre-existing, unrelated
partition-mate (fail-fast); neither file in this PR failed.

- [x] AMD CI passes on the registered files (both verified PASS on MI325)
