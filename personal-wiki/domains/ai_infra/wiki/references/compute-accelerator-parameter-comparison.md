...
- Aggregate records must be normalized before card-to-card comparison: AWS
  ...
  multi-chip or system-level totals. The structured catalog resolves the
  Trn2 instance cloud offering count and aggregate accelerator memory
  (16 Trainium2 chips and 1536 GB for `trn2.48xlarge` and `trn2u.48xlarge`)
  and the Atlas 800T A3 training-server aggregate count and memory
  (8 Ascend 910 processors and 1024 GB from 8 x 128 GB on-chip memory).
  UltraServer 6 TB memory, Atlas 800T A3 aggregate compute/bandwidth/network
  values, and other system-level totals remain comparison-only boundary
  evidence. Use these rows for capacity
...
| Huawei Atlas 800T A3 | 10U training supernode server | 8 Ascend 910 processors is resolved as an aggregate system count; up to 6.0 PFLOPS FP16 and 12.0 POPS INT8 remain comparison-only system totals | 8 x 128 GB on-chip memory is resolved as 1024 GB aggregate memory; 3.2 TB/s memory bandwidth remains comparison-only | D2D 784 GB/s bidirectional, 8 x 400GE RoCE direct, 56 x 400GE bus-protocol interfaces, up to 5 PCIe 5.0 slots, storage, power, cooling, fans, dimensions, and operating-temperature remain aggregate boundary evidence | [resolved specs](../../data/compute_accelerators/resolved/sample-resolved-specs.yaml), [raw](../../raw/crawler/compute-accelerators-huawei-atlas-800t-a3/20260628T060648300656Z-e-huawei-com-cn-products-computing-ascend-atlas-800t-a3-4a689659c8.md) |
...
| Dayu Paratus 1.0 | DPU product page | qualitative software/offload boundary only: Linux on ARM SoC plus DPDK/SPDK development kits and host-side function offload wording; no numeric compute or network parameter captured | virtualization-network, storage-client, OpenStack, and Kubernetes integration components for bare-metal, VM, and container cloud services; no memory, power, host-interface, benchmark, operation, or SLO field captured | [raw](../../raw/crawler/compute-accelerators-dayu-paratus1/20260628T060445164573Z-www-dayudpu-com-product-paratus1-70c58cfadc.md), [gap proof](../../manifest-ai-infra-expansion-continuation-20260708-parent-22-gap-proof.json) |
| Dayu Paratus 2.0 | DPU product page | qualitative software/offload boundary only: ARM SoC + FPGA architecture, same software development/runtime environment as Paratus 1.0, FPGA network data-path processing, HPRT RDMA-adjacent protocol wording, end-to-end network data encryption, and upper-layer application behavior analysis; no numeric compute or network parameter captured | OpenStack/Kubernetes cloud-integration component wording persists; no memory, power, host-interface, benchmark, operation, deployment, or SLO field captured | [raw](../../raw/crawler/compute-accelerators-dayu-paratus2/20260628T060445547733Z-www-dayudpu-com-product-paratus2-24678a2fdd.md), [gap proof](../../manifest-ai-infra-expansion-continuation-20260708-parent-22-gap-proof.json) |
...
| Dayu Paratus 1.0 | DPU product page | Linux on ARM SoC, DPDK/SPDK development kits, and host-side function offload wording; virtualization-network, storage-client, OpenStack, and Kubernetes integration components for bare-metal, VM, and container cloud services | no structured memory, host-interface, power, bandwidth, IOPS, latency, benchmark, operation, SLO, or deployment field is resolved | [raw](../../raw/crawler/compute-accelerators-dayu-paratus1/20260628T060445164573Z-www-dayudpu-com-product-paratus1-70c58cfadc.md), [gap proof](../../manifest-ai-infra-expansion-continuation-20260708-parent-22-gap-proof.json) |
| Dayu Paratus 2.0 | ARM SoC + FPGA DPU product page | same software development/runtime environment as Paratus 1.0; FPGA network data-path processing; Dayu HPRT RDMA-adjacent protocol wording; end-to-end network data encryption; upper-layer application behavior-analysis wording | no structured memory, host-interface, power, bandwidth, IOPS, latency, benchmark, operation, SLO, or deployment field is resolved | [raw](../../raw/crawler/compute-accelerators-dayu-paratus2/20260628T060445547733Z-www-dayudpu-com-product-paratus2-24678a2fdd.md), [gap proof](../../manifest-ai-infra-expansion-continuation-20260708-parent-22-gap-proof.json) |
...
Kunlunxin R480-X8 reports an 8-module aggregate. The structured catalog now
normalizes only the safe aggregate count and memory fields for AWS Trn2 and
Atlas 800T A3; compute, bandwidth, fabric, storage, and deployment claims still
need separate schema boundaries before card-to-card comparison.
