...
The scan reuses existing local evidence rather than recrawling the baseline-dirty 20260706 compute accelerator captures. Current coverage is strongest for NCCL communication, SGLang inference-runtime evidence, Kubernetes-native scheduling corpora, and compute accelerator raw inventory. Data/RAG/vector infrastructure and security/governance/cost remain the weakest layers.

# Layer Status

| Layer | Current local coverage | Next gap |
| --- | --- | --- |
| `training-distributed` | NCCL release notes and closed GitHub issues cover collective communication and operational failures. | Add source-backed training framework, checkpointing, and elastic training pages. |
...
| `eval-observability-reliability` | The Hitchhiker paper covers agentic evaluation concepts, while SGLang and NCCL issue corpora supply reliability and debugging signals. | Add dedicated observability, tracing, profiling, benchmark, SLO, and incident sources. |
| `security-governance-cost` | Current evidence is adjacent: NCCL GPU sharing/virtualization issues, DPU security positioning, and Kueue quota-aware scheduling. | Add tenant isolation, MIG/vGPU, confidential computing, cost attribution, and capacity planning sources. |
...
| `network-storage-cluster` | NCCL network/RDMA issue evidence and DPU/SmartNIC captures cover some cluster fabric and storage-offload signals. | Add InfiniBand, RoCE, Spectrum-X, EFA, Lustre, Weka, Ceph, NVMe-oF, and parallel filesystem evidence. |
