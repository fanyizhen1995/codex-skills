...
The scan reuses existing local evidence and now treats the tracked 20260706 compute accelerator baseline as reconciled hardware evidence rather than unreconciled input. Current coverage is strongest for NCCL communication and NCCL-adjacent observability/fabric evidence, Kubernetes-native scheduling corpora, and compute accelerator raw inventory. Distributed training now has source-backed framework lifecycle, sharding, checkpointing, elastic restart, Ray Train, Kubeflow Trainer, and KubeRay coverage beyond NCCL, plus task-3 issue-level evidence for NCCL abort/failover paths, version/configuration-sensitive failures, and one Ray/H200/NVSwitch broadcast hang report. Orchestration/scheduling now has source-backed Ray/KubeRay, Slurm GPU/GRES, Kubernetes device plugin, NVIDIA and AMD GPU operator, Kubeflow Trainer, and Kueue/ResourceQuota admission evidence beyond the existing Kubernetes/Volcano/Kueue corpora, plus task-3 issue-level signals for Kueue admission/preemption/ResourceFlavor behavior and Volcano gang scheduling, GPU sharing, preemption, scheduler panic, and upgrade/quota failures. Inference-runtime now has multi-project primary-source coverage through vLLM, TensorRT-LLM, Triton Inference Server, llama.cpp, ONNX Runtime GenAI, Ray Serve autoscaling/resource/placement-group controls, Knative autoscaling and revision traffic splitting, local TensorRT/vLLM captures, PegaFlow external KV-cache operations, vLLM rollout weight-sync controls, and SGLang router/PD disaggregation operational issue evidence. Data/RAG/vector infrastructure has partial primary-source coverage through Milvus, Qdrant, Weaviate, pgvector, and FAISS source notes plus Ray Data, Flink, Kafka Connect/Streams, Airflow, Dagster, Prefect, TEI, DataHub/OpenLineage, Iceberg, Delta Lake, Hudi, Langfuse pipeline sources, vector-store delete/TTL controls, Azure AI Search deletion detection, document-level access control, chunk permission projection, PII masking, and Langfuse dataset versioning. Evaluation/observability/reliability now has general LLM tracing, evaluation-harness, dataset-versioning, SLO/alert-routing mechanics, LLM application alert surfaces, NVIDIA platform profiling, and AMD/Gaudi observability coverage through OpenTelemetry GenAI, LangSmith, Phoenix, Ragas, lm-evaluation-harness, Langfuse, Prometheus, Alertmanager, Grafana, DCGM, Nsight Systems, AMD Device Metrics Exporter, ROCm/AMD SMI, and Intel Gaudi Prometheus exporter source notes. Security/governance/cost now has source-backed coverage for tenant isolation, accelerator sharing, quota governance, RAG access policy hooks, PII controls, metadata policy, vector-store RBAC/tenancy, evaluation dataset versioning, workload cost allocation, and accelerator capacity planning. Network/storage/cluster coverage now adds EFA, FSx for Lustre, WEKA, Ceph, SPDK NVMe-oF, AWS HyperPod topology/checkpoint/resiliency mechanics, and MLCommons Storage benchmark-method evidence while reusing Spectrum-X/RoCE and local DPU/NVMe-oF evidence. The layers remain partial where KServe-specific rollout/autoscaling proof, production postmortems, product-specific benchmark submissions, field-operation, cross-cloud chargeback, end-to-end policy enforcement, retrieval-quality alerting, validated service-specific SLOs, TPU/XPU production observability, and full source-to-vector/cache/eval propagation evidence is still thin.
...
| `inference-runtime` | SGLang issue/PR corpora and CUDA Green Context evidence cover serving runtime behavior and compatibility risks; [Inference Runtime Infrastructure](inference-runtime-infrastructure.md) adds vLLM, TensorRT-LLM, Triton Inference Server, llama.cpp, ONNX Runtime GenAI, Ray Serve autoscaling/resource/placement-group controls, Knative autoscaling and revision traffic splitting, local TensorRT/vLLM runtime evidence, PegaFlow external KV-cache operations, vLLM rollout weight-sync control, SGLang router abort observability, PD disaggregation failure handling, benchmark/profiling leads, and warmup/startup regression leads. | Add KServe-specific autoscaling/canary proof, production failure and rollback evidence around deployment controls, full production postmortem, live trace/profiling-to-SLO, and full SGLang API refresh evidence where needed. |
...
| `security-governance-cost` | [Security Governance Cost Infrastructure](security-governance-cost-infrastructure.md) covers Quantum InfiniBand tenant isolation, MIG/vGPU accelerator sharing, NVIDIA confidential-computing attestation, Kubernetes ResourceQuota, Kueue ClusterQueue/ResourceFlavor/cohort governance, Azure AI Search document-level access control and PII masking, DataHub metadata policy, Weaviate RBAC/multi-tenancy, Langfuse dataset versioning, OpenCost workload cost allocation, AWS accelerator capacity/service-quota planning, and NCCL 2.22 cost-estimation boundaries. | Add cross-cloud chargeback, full end-to-end policy enforcement evidence, and incident/postmortem evidence. |
