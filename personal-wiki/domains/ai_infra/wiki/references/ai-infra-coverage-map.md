...
| `inference-runtime` | SGLang issue/PR corpora and CUDA Green Context evidence cover serving runtime behavior and compatibility risks; [Inference Runtime Infrastructure](inference-runtime-infrastructure.md) adds vLLM, TensorRT-LLM, Triton Inference Server, llama.cpp, ONNX Runtime GenAI, and local TensorRT/vLLM runtime evidence for batching, KV-cache lifecycle, model repository/config loading, OpenAI-compatible serving, provider selection, distributed inference knobs, PegaFlow external KV-cache operations, vLLM rollout weight-sync control, SGLang router abort observability, PD disaggregation failure handling, benchmark/profiling leads, and warmup/startup regression leads. | Add autoscaling, model-placement, canary rollout, full production postmortem, live trace/profiling-to-SLO, and full SGLang API refresh evidence where needed. |
