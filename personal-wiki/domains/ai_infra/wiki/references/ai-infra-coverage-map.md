...
| `network-storage-cluster` | [Network Storage Cluster Infrastructure](network-storage-cluster-infrastructure.md) adds AWS EFA, FSx for Lustre, WEKA, Ceph, SPDK NVMe-oF, AWS HyperPod topology-aware scheduling, EFA health checks, node replacement, checkpoint-aware resume, and MLCommons Storage benchmark-method evidence, while NCCL issue evidence, DPU/SmartNIC captures, and NCCL technical-blog captures cover RoCE, Spectrum-X, BGP PIC convergence, ECN/DC-QCN, SHARP, NIC Fusion, and fabric telemetry. R9 task 3 adds blocked-source evidence that no exact MLCommons measured-result artifact was available locally and bounded GitHub probes failed with DNS resolution errors. R10 task 3 keeps Resnics S1100 IOPS/latency as product-specific catalog fields, not a general NVMe-oF benchmark claim; Corigine Agilio CX and Yusur K2-Pro/SWIFT rows are likewise product-specific SmartNIC/DPU catalog or comparison slices, not benchmark, protocol, or operations claims. | Add complete production incident/postmortem sources, exact product/provider benchmark submissions or case studies with full environment and measured results, and broader non-AWS/non-NVIDIA fabric operations evidence. |
